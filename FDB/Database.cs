...
namespace FDB
{
    internal static class Database
    {
        ...
        private static DataTable GetEntitiesFromQuery(Table table, string rows, string queryColumn, string query)
        {
            ...
            {
                cmd.CommandText = "select " + rows + " from " + tables[(int)table] + " where " + queryColumn + " ilike '%" + query + "%';";
            }
        ...
        public static DataTable GetActorsFromQuery(string query)
        {
            return GetEntitiesFromQuery
                (
                    Table.Actor,
                    "concat(trim(actor.act_fname), ' ', trim(actor.act_lname)) as Actor",
        ...
        public static int InsertMovieToDatabase(string title, int year, int play_time, string resume, string img_path)
        {
            NpgsqlConnection connection = new NpgsqlConnection(connectionString);
            connection.Open();
            NpgsqlCommand cmd = new NpgsqlCommand
        ...
        private static bool InsertActorToMovie(int mov_id, int act_id, string role)
        {
            NpgsqlConnection connection = new NpgsqlConnection(connectionString);
            connection.Open();
            NpgsqlCommand cmd = new NpgsqlCommand
        ...
        private static bool RemoveActorFromMovie(int mov_id, int act_id)
        {
            NpgsqlConnection connection = new NpgsqlConnection(connectionString);
            connection.Open();
            NpgsqlCommand cmd = new NpgsqlCommand
